$(document).ready(function () {
    configurarMascaras();
    preencherFormularioSeObjExistir();
    configurarEventos();
...
function configurarMascaras() {
    $('#BeneficiarioCPF, #CPF').mask('000.000.000-00', { reverse: true });
}

function preencherFormularioSeObjExistir() {
    if (obj) {
        preencherCamposFormulario(obj);
        $('#tabelaBeneficiarios tbody').empty();  // Certifique-se de que a tabela seja limpa antes de preenchê-la
        preencherTabelaBeneficiarios(obj.Beneficiarios);
    }
}

function preencherCamposFormulario(obj) {
    const campos = ['Nome', 'CEP', 'Email', 'Sobrenome', 'Nacionalidade', 'Estado', 'Cidade', 'Logradouro', 'Telefone', 'CPF'];
    campos.forEach(campo => {
        $('#formCadastro #' + campo).val(obj[campo]);
    });
}
...
function preencherTabelaBeneficiarios(beneficiarios) {
    beneficiarios.forEach(function (beneficiario) {
        ...
        adicionarLinhaTabela(beneficiario.Id, cpf, beneficiario.Nome);
    });
}

function formatarCPF(cpf) {
    return cpf.mask('000.000.000-00', { reverse: true });
}
...
function adicionarBeneficiario() {
    var cpf = $('#BeneficiarioCPF').mask('000.000.000-00', { reverse: true }).val();
